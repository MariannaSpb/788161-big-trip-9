...
export const getInfoTrip = () => {
  ...
  <p class="trip-info__dates">${dates.length > 2 ? `${new Date(events[0].schedule.date).toLocaleDateString(`en-US`, {day: `numeric`, month: `short`})} — ${new Date(dates[dates.length - 1]).toLocaleDateString(`en-US`, {day: `numeric`})}` : `${dates.join(`—`)}`}</p>
</div>`;
};
